...
General Information)").c_str(),
	...
	smtCheckerOptions.add_options()
		(
			...
			po::value<string>()->value_name("none, bmc, chc, all"),
			"Select desired model checker engine."
		)
	;
...
bool CommandLineInterface::processInput()
{
	...
	{
		string engine = m_args[g_argModelCheckerEngine].as<string>();
		if (engine == "none")
			m_modelCheckerEngine = ModelCheckerEngine::None();
		else if (engine == "bmc")
			m_modelCheckerEngine = ModelCheckerEngine::BMC();
		else if (engine == "chc")
			m_modelCheckerEngine = ModelCheckerEngine::CHC();
		else if (engine == "all")
			m_modelCheckerEngine = ModelCheckerEngine::All();
		else
		{
			serr() << "Invalid option for --" << g_argModelCheckerEngine << ": " << engine << endl;
			return false;
		}
	}
